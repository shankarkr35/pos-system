            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="custom-data-table" class="table table-bordered table-striped">
                  ...
                  <tbody>
                      <?php $cnt=0; foreach($sliders as $slides):?>
                        <tr id="row<?php echo $slides->id?>">
                            <td><?php echo $cnt=$cnt+1;?></td>
                            <td><img src="<?php echo base_url('uploads/homesliders/medium/').$slides->image;?>" width="250"></td>
                            <td>
                                <select class="form-control status-change" id="<?php echo $slides->id?>">
                                    <option value="1" <?php echo (($slides->status==1)?'selected':'')?>>Published</option>
                                    <option value="0" <?php echo (($slides->status==0)?'selected':'')?>>Draft</option>
                                </select>
                            </td>
                          <td>
                                <a href="<?php echo base_url('edit-home-slider/').$slides->id?>" class="btn bg-gradient-success btn-sm">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a class="btn bg-gradient-danger btn-sm" onclick="checkdelete(<?php echo $slides->id?>)"><i class="fas fa-trash-alt"></i></a>
                          </td>
                        </tr>
                      <?php endforeach?>
                  </tbody>
...
<script>
    $(function(){
        ...
        if ( sessionStorage.getItem('updated') ) {
           swal({
              title: "",
              text: "Slider Details updated successfully.!",
              type: "success",
